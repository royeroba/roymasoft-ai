...
export function project({ target, agents = ALL_AGENTS, log = () => {} }) {
	...
	/** Writes content wrapped in our idempotent block markers, preserving whatever else is in the
	...
	const writeBlock = (relTarget, block) => {
		const abs = join(TARGET, relTarget);
		if (existsSync(abs)) {
			...
			} else if (current.includes('Generated by roymasoft-ai')) {
	...
	const writeOwned = (relTarget, content) => {
		const abs = join(TARGET, relTarget);
		if (existsSync(abs) && !readFileSync(abs, 'utf8').includes(SKILL_MARK)) {
	...
	if (selected.has('codex')) {
		for (const name of skillNames()) {
			for (const rel of skillFiles(name)) {
				const content = stripAgentBlocks(readFileSync(join(HARNESS, 'skills', name, rel), 'utf8'), 'codex');
				writeOwned(`.codex/skills/${name}/${rel}`, content);
			}
		}
	}
	...
	if (selected.has('claude')) {
		...
		if (claudeCurrent === null || claudeCurrent.trim() === 'AGENTS.md') {
			// Nothing there yet, or already exactly our pointer — keep the simple one-line form.
		...
		for (const file of listFiles('agents')) {
			...
			writeOwned(`.claude/agents/${file}`, stripAgentBlocks(read(`agents/${file}`), 'claude'));
		}
		for (const name of skillNames()) {
			for (const rel of skillFiles(name)) {
				const content = stripAgentBlocks(readFileSync(join(HARNESS, 'skills', name, rel), 'utf8'), 'claude');
				writeOwned(`.claude/skills/${name}/${rel}`, content);
			}
		}
	...
	if (selected.has('antigravity')) {
		...
		for (const name of skillNames()) {
			for (const rel of skillFiles(name)) {
				const content = stripAgentBlocks(readFileSync(join(HARNESS, 'skills', name, rel), 'utf8'), 'antigravity');
				writeOwned(`.antigravity/skills/${name}/${rel}`, content);
			}
		}
	}
